...
首次打开时，Shortcutype 会先用一句话说明完整规则：**动作是问题，快捷键是你的答案**。随后用当前平台对应的组合键完成三步入门：

1. **认识答案：**看着答案按一次“复制”。
2. **照着完成：**沿用相同输入方式完成“在文件中查找”。
3. **自己回忆：**隐藏答案后再次完成“复制”。

接着选择你常用的工具，例如 VS Code、Vim、Git 或开发者工具，应用会立即开始一组相关的 10 题训练。入门可以跳过，也可以从“高级设置”或命令面板重播；已有训练记录的老用户不会被强制带回首次流程。

完成入门后，准备页先问“今天想练什么”，直接提供继续训练、5 分钟热身、弱项复习和所选工具训练；完整配置仍然只需一步即可打开。
...
安装 Node.js 20 或更高版本后，全新检出只需一条命令：

```bash
npm start
```

缺少依赖时，启动命令会按照 lockfile 自动安装，然后启动本地 Vite 服务。
...
完整发布门禁使用 `npm run verify`。

Vitest 与 Testing Library 已覆盖输入解析、多段序列、别名、自适应调度、入门状态迁移、平台对应组合键、v1→v2 迁移、计时边界、命令面板和键盘会话状态流。

## 视觉验证

真实浏览器审查过的关键状态保存在 [`docs/screenshots`](screenshots)：准备、专注训练、多段序列、错误、命令面板、结果、浅色主题和移动端。[`onboarding` 截图组](screenshots/onboarding)记录了首次说明、认识答案、跟随输入、独立回忆、错误对比、工具选择、个性化准备页和移动端准备页。

Shortcutype 借鉴优秀打字工具“专注、即时”的原则，但交互模型、视觉系统、Chord Trace、调度机制和代码实现均为独立原创。

更多资料：[架构说明](ARCHITECTURE.md)、[用户指南](USER_GUIDE.md)、[已知限制](KNOWN_LIMITATIONS.md)和[加固进度](../progress.md)。
